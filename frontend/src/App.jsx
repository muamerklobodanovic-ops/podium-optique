...
import { 
  ...
  Glasses, Ruler, ChevronRight, Layers, Sun, Monitor, Sparkles, Tag, Eye, EyeOff, Settings, X, Save, Store, Image as ImageIcon, Upload, Car, Database
} from 'lucide-react';
...
function App() {
  ...
  const [debugUrl, setDebugUrl] = useState(""); // Pour afficher l'URL appelée en cas de pépin
  ...
  // ⚠️ Assurez-vous que cette URL est correcte (celle de votre Render)
  ...
  const fetchData = (ignoreFilters = false) => {
    ...
    // Si on est sur internet mais que l'URL est celle par défaut, on prévient
    if (!isLocal && API_URL.includes("api-podium-optique")) {
       setDebugUrl(API_URL); // On affiche l'URL pour debug
    }

    // Si ignoreFilters est VRAI, on ne met aucun paramètre pour récupérer TOUT
    ...
    axios.get(API_URL, { params })
      ...
      .catch(err => {
        ...
        setLenses(MOCK_LENSES); // Fallback
        setError("Connexion au serveur échouée. Mode démo activé.");
        setLoading(false);
      });
  };

  // ... (Reste des handlers inchangés: handleChange, handleLogoUpload, etc.)
      ...
      <main className="flex-1 flex overflow-hidden relative z-0">
        ...
        <section className="flex-1 p-8 overflow-y-auto bg-slate-50">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-8 flex flex-wrap gap-3 text-sm items-center font-bold text-slate-500">
               {/* ... (Affichage des filtres inchangé) ... */}
               <span>FILTRES :</span>
            ...
            {error && (
              <div className="bg-red-50 text-red-600 p-6 rounded-2xl mb-8 border border-red-200 flex items-center gap-4 font-bold">
                ...
                <div>
                  <p>{error}</p>
                  {debugUrl && <p className="text-xs font-mono mt-2 text-red-400">URL Ciblée : {debugUrl}</p>}
                </div>
              </div>
            )}

            {/* État "Aucun résultat" avec Diagnostic */}
            {lenses.length === 0 && !loading && !error && (
              <div className="col-span-3 py-20 text-center">
                ...
                <p className="text-sm text-slate-400 font-bold mb-6">
                  LA BASE DE DONNÉES NE CONTIENT AUCUN PRODUIT CORRESPONDANT EXACTEMENT À :<br/>
                  MARQUE={formData.brand} | TYPE={formData.type} | INDICE={formData.materialIndex} | TRAITEMENT={formData.coating}
                </p>
                <button 
                  onClick={() => fetchData(true)} 
                  className="px-6 py-3 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-xl transition-colors flex items-center gap-2 mx-auto"
                >
                  <Database className="w-4 h-4"/> VOIR TOUT LE CATALOGUE (TEST)
                </button>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
              {lenses.map((lens, index) => {
                ...
                // On s'assure que brandLogo reçoit une string valide
                const brandName = lens.brand ? lens.brand.toUpperCase() : "INCONNU";
                  ...
                  <div key={lens.id} className={`group bg-white rounded-3xl hover:-translate-y-2 border-2 transition-all duration-300 overflow-hidden relative ${style.border}`}>
                     ...
                     <div className="p-8 pt-14 border-b border-slate-50 relative">
                        ...
                        <p className="text-sm text-slate-500 font-bold flex items-center gap-3">
                           {brandName} <span className="w-1.5 h-1.5 rounded-full bg-slate-300"/> INDICE {lens.index_mat || formData.materialIndex}
                        </p>
                     </div>
                     <div className="p-8 bg-slate-50/50 group-hover:bg-white transition-colors space-y-6">
                        {showMargins ? (
                          <>
                            ...
                            <div className="pt-2">
                              <div className="flex justify-between items-end mb-2"><span className="text-xs font-bold text-green-700 tracking-wide">MARGE NETTE</span><span className="text-xs font-bold text-green-700 bg-green-100 px-3 py-1 rounded-lg">{((lens.margin / lens.sellingPrice) * 100).toFixed(0)}%</span></div>
        ...
        {showSettings && (
          <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex justify-center items-center p-4">
             {/* ... (Contenu Modale Settings inchangé pour gain de place, mais inclus dans le fichier final) ... */}
