...
// --- VERSION APPLICATION ---
const APP_VERSION = "3.43"; // Fix Settings Crash (Safe Merge & Access)
...
function App() {
  ...
  // --- INITIALISATION ROBUSTE DES PARAMÈTRES (FUSION PROFONDE) ---
  const [userSettings, setUserSettings] = useState(() => {
    try { 
      const saved = localStorage.getItem("optique_user_settings");
      if (saved) {
          const parsed = JSON.parse(saved);
          // On fusionne ce qui est sauvegardé avec les valeurs par défaut pour combler les manques
          return {
              ...DEFAULT_SETTINGS,
              ...parsed,
      ...
      console.error("Erreur lecture settings, reset défaut.", e);
      return DEFAULT_SETTINGS; 
    }
  });
  ...
  useEffect(() => {
    ...
    if (safeLenses.length > 0) {
       ...
       if (formData.network === 'HORS_RESEAU') {
          // SÉCURITÉ PRICING : Utilisation des valeurs par défaut si manquant dans userSettings
  ...
  // FIX: Fonction de mise à jour de prix robuste avec protection
  const handlePriceRuleChange = (category, field, value) => { 
      setUserSettings(prev => {
          // On s'assure que 'pricing' et 'category' existent
          const currentPricing = prev.pricing || DEFAULT_SETTINGS.pricing;
          const currentCategory = currentPricing[category] || DEFAULT_SETTINGS.pricing[category];
  ...
  // SÉCURITÉ AFFICHAGE MODALE SETTINGS
  // On utilise une variable locale 'safePricing' garantie non-nulle
  const safePricing = {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgClass} ${textClass} relative font-['Arial'] uppercase transition-colors duration-300`}>
      ...
      {/* MODALE SETTINGS AVEC SÉCURITÉ RENFORCÉE */}
      {showSettings && (
