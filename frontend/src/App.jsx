...
// --- VERSION APPLICATION ---
const APP_VERSION = "4.14"; // Filtres Permissifs (Affichage Garanti)
...
function App() {
       ...
       if (formData.type) { 
           ...
           if (targetType === 'PROGRESSIF_INTERIEUR') {
               workingList = workingList.filter(l => {
                   ...
                   return type === 'PROGRESSIF_INTERIEUR' || type.includes('INTERIEUR') || type.includes('DEGRESSIF');
               });
           } else { 
               workingList = workingList.filter(l => {
                   const t = cleanText(l.type);
                   // Si vide, on montre par défaut pour éviter écran vide
                   if (t === "") return true;
                   return t === targetType || t.includes(targetType);
               });
           } 
       }
  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgClass} ${textClass} relative font-['Arial'] uppercase transition-colors duration-300`}>
      {/* ... (Contenu JSX identique mais avec les props correctes) ... */}
