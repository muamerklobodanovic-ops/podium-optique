...
function App() {
  ...
  // Etat du formulaire
  // MODIFICATION ICI : Sphère et Addition à 0.00 par défaut
  const [formData, setFormData] = useState({
    network: 'HORS_RESEAU',
    brand: 'ORUS',         
    type: 'PROGRESSIF',
    sphere: 0.00,    // Valeur par défaut modifiée
    cylinder: 0.00,
    addition: 0.00,  // Valeur par défaut modifiée
    materialIndex: '1.60',
    coating: 'MISTRAL', 
    cleanOption: false, 
    myopiaControl: false,
  ...
  // ⚠️ REMPLACEZ CECI PAR VOTRE URL RENDER DE PRODUCTION (API)
  ...
  const fetchData = () => {
    ...
    // SÉCURITÉ : Si l'URL est par défaut, on utilise les MOCKs pour éviter le crash
    if (!isLocal && API_URL.includes("VOTRE-URL-RENDER-ICI")) {
      console.warn("URL API non configurée. Mode démo.");
