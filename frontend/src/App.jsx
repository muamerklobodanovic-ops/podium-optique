...
const DEFAULT_SETTINGS = {
    ...
    disabledBrands: [], // Liste des ID de marques masquées
    pricing: {
        uniStock: { x: 2.5, b: 20 },   
        uniFab: { x: 3.0, b: 30 },     
        prog: { x: 3.2, b: 50 },       
...
// --- APP PRINCIPALE ---
function App() {
  const [user, setUser] = useState(() => { const s = sessionStorage.getItem("optique_user"); return s ? JSON.parse(s) : null; });
  ...
  const lensTypes = [ { id: 'UNIFOCAL', label: 'UNIFOCAL' }, { id: 'PROGRESSIF', label: 'PROGRESSIF' }, { id: 'DEGRESSIF', label: 'DÉGRESSIF' }, { id: 'MULTIFOCAL', label: 'MULTIFOCAL' }, { id: "PROGRESSIF_INTERIEUR", label: "PROG. INTÉRIEUR" } ];
    ...
    try { 
        ...
        if(saved) { 
            ...
            return { 
                ...
                // On s'assure que disabledBrands existe
                disabledBrands: p.disabledBrands || [] 
            }; 
        } 
  ...
  // FILTRAGE MARQUES (Réseau + Désactivées + ON/OFF Settings)
  const getFilteredBrandsList = () => {
      // 1. Marques désactivées par l'utilisateur (ON/OFF dans Settings)
      const userDisabled = userSettings.disabledBrands || [];
      let available = brands.filter(b => !userDisabled.includes(b.id));

      // 2. Restrictions par Réseau
  ...
  useEffect(() => {
    ...
    if (safeLenses.length > 0) {
       ...
       if (formData.brand && formData.brand !== '') { 
           ...
           // Si "TOUTES", on applique les restrictions
           ...
           // ET on applique le filtre ON/OFF utilisateur
           if (userSettings.disabledBrands && userSettings.disabledBrands.length > 0) {
  ...
  }, [lenses, formData, userSettings.pricing, userSettings.disabledBrands]); // Ajout dependance disabledBrands
  ...
  const handleReset = () => {
      if(window.confirm("Tout remettre à zéro ?")) {
          ...
          // On ne reload pas pour ne pas déconnecter
  ...
  const handleLogout = () => { setUser(null); sessionStorage.clear(); window.location.reload(); };
  ...
  // ... (Handlers Save/Delete/Upload inchangés) ...
  ...
  const deleteOffer = (id) => {
      if (window.confirm("⚠️ ATTENTION: Cette action est irréversible. Supprimer ce dossier ?")) {
          axios.delete(`${SAVE_URL}/${id}`).then(() => { alert("Dossier supprimé."); fetchHistory(); }).catch(err => { const msg = err.response ? `Erreur ${err.response.status}` : err.message; alert(`Erreur suppression : ${msg}`); });
      }
  };
  const triggerFileUpload = () => {
      ...
      .then(res => { alert(`✅ Succès ! ${res.data.count} verres.`); fetchData(); })
  ...
  const triggerUserUpload = () => {
      ...
      axios.post(`${baseBackendUrl}/upload-users`, data).then(res => alert(`✅ ${res.data.count} utilisateurs`)).catch(err => alert("Erreur upload")).finally(() => setSyncLoading(false));
  };
  ...
  // NOUVEAU : Toggle Marque ON/OFF
  const toggleBrand = (brandId) => {
      setUserSettings(prev => {
          const currentDisabled = prev.disabledBrands || [];
          const newDisabled = currentDisabled.includes(brandId) 
              ? currentDisabled.filter(id => id !== brandId) // On réactive
              : [...currentDisabled, brandId]; // On désactive
  ...
  return (
    <div className={`min-h-screen flex flex-col ${bgClass} ${textClass} relative font-['Arial'] uppercase transition-colors duration-300`}>
      ...
      {/* FOOTER DEVIS (Identique v3.99.1) */}
      {selectedLens && (
          <div className="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] z-50 p-4 animate-in slide-in-from-bottom-10 text-slate-800">
              <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4"><div className="bg-blue-100 p-3 rounded-xl text-blue-600"><Glasses className="w-6 h-6"/></div><div><div className="text-[10px] font-bold text-slate-400 mt-1">RÉFÉRENCE DE COMMANDE</div><div className="font-mono text-xs bg-slate-100 p-1 rounded cursor-pointer hover:bg-blue-100 transition-colors select-all" onClick={() => { navigator.clipboard.writeText(selectedLens.commercial_code); alert("Référence copiée !"); }} title="Cliquer pour copier">{selectedLens.commercial_code || "N/A"}</div><div className="font-bold text-slate-800 text-sm leading-tight mt-1">{selectedLens.name}</div></div></div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex justify-center items-center p-4" onClick={(e) => { if(e.target === e.currentTarget) setShowSettings(false); }}>
           <div className="bg-white w-full max-w-2xl rounded-3xl p-8 shadow-2xl max-h-[90vh] overflow-y-auto text-slate-800">
              ...
              {/* SECTION ADMIN (Seulement pour rôle 'admin') */}
              {user?.role === 'admin' && (
                <div className="bg-orange-50 p-4 rounded-xl border border-orange-200 mb-6">
                    ...
                    <div className="mb-4">
                         ...
                         <div className="flex gap-2"><input type="file" accept=".xlsx" onChange={(e) => setUserFile(e.target.files[0])} className="flex-1 text-xs bg-white"/><button onClick={triggerUserUpload} disabled={syncLoading} className="bg-orange-600 text-white px-4 py-2 rounded text-xs font-bold">{syncLoading ? "..." : "ENVOYER"}</button></div>
                    </div>

                    <div className="mb-4">
                         <label className="block text-xs font-bold text-slate-600 mb-2">IMPORTER CATALOGUE VERRES</label>
                         <div className="flex gap-2"><input type="file" accept=".xlsx" onChange={(e) => setUploadFile(e.target.files[0])} className="flex-1 text-xs bg-white"/><button onClick={triggerFileUpload} disabled={syncLoading} className="bg-orange-600 text-white px-4 py-2 rounded text-xs font-bold">{syncLoading ? "..." : "ENVOYER"}</button></div>
                    </div>
                    
                    <div className="flex justify-between items-center">
                        <span className="text-xs">État Base de Données</span>
                        <button onClick={checkDatabase} className="bg-white border border-orange-300 px-3 py-1 rounded text-xs font-bold text-orange-700">VÉRIFIER</button>
                    </div>
                </div>
              )}
              ...
              <div className="mb-8 p-4 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                      {brands.filter(b => b.id !== '').map(b => {
                          const isDisabled = userSettings.disabledBrands?.includes(b.id);
                          return (
                              <button key={b.id} onClick={() => toggleBrand(b.id)} className={`px-3 py-2 rounded-lg text-xs font-bold border ${isDisabled ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-blue-600 text-white border-blue-600'}`}>
